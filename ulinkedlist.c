...
int addHeadToEmpty(ulinkedlist_t *list, void *val) {
  list->length++;
  unode_t *head = malloc(sizeof(unode_t));
  if (head == NULL) {
    return -1;
  }
  head->val = val;
...
int addAtTail(ulinkedlist_t *list, void *val) {
  ...
  unode_t *node = malloc(sizeof(unode_t));
  if (node == NULL) {
    return -1;
  }
  node->val = val;
...
unode_t *findNode(ulinkedlist_t *list, int idx) {
...
int addAtIndex(ulinkedlist_t *list, void *val, int idx) {
  ...
  unode_t *node = findNode(list, idx - 1);
  unode_t *new_node = malloc(sizeof(unode_t));
  if (new_node == NULL) {
    return -1;
  }
  new_node->val = val;
...
void *getByIndex(ulinkedlist_t *list, int idx) {
  ...
  unode_t *node = findNode(list, idx);
  return node->val;
}
...
int deleteAtIndex(ulinkedlist_t *list, int idx) {
  ...
  if (idx == 0) {
    ...
    unode_t *node = findNode(list, list->length - 2);
    unode_t *next = node->next;
    node->next = NULL;
    list->tail = node;
    list->length--;
    free(next);
